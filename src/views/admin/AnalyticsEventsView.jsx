...
function Table({ data }) {
    function Row({ datum }) {
        ...
        function Profile({ name, hasEditAccess }) {
            return <div className="profile">
                <div className="info">
                    <div><span>{name}</span>{hasEditAccess && <span></span>}</div>
                </div>
            </div>
        }
        return <tr>
            <td>{id}</td>
            <td><img src={picture} alt={email} /></td>
            <td><Profile picture={picture} name={name} email={email} hasEditAccess={hasEditAccess} /></td>
            <td>{email}</td>
    ...
    return (
        <table>
            ...
            <tbody>
                {data?.map(datum => <Row key={datum.id} datum={datum} />)}
            </tbody>
        </table>
    )
}
            ...
            <div className="list-item">
                ...
                <div className="info">
                    ...
                    <div className="event-time">
                        <Event event={event} />
                        <div style={{ display: 'inline-block' }}>{new Date(created_at).toLocaleString()}</div>
                    </div>
                </div>
            </div>
        )
    }
    return <div className="list">{data?.map(datum => <ListItem key={datum?.id} datum={datum} />)}</div>
}
