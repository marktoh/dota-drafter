...
const MyChart = ({ data }) => {
    const formattedData = data?.map(datum => ({ ...datum, day: new Date(datum?.day).toLocaleDateString() })).reverse();
    return (
        <ResponsiveContainer width="100%" height="100%">
            ...
            >
                ...
                <YAxis />
                <Tooltip />
                <Area type="monotone" dataKey="total_visits" stroke="var(--gold)" fill="var(--gold)" strokeWidth={2} />
            </AreaChart>
        </ResponsiveContainer>
    )
}
