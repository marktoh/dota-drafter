...
function filter(heroes, searchInput) {
    if (searchInput?.length > 0) {
        heroes = heroes.filter(hero => formatName(hero?.localized_name)?.includes(formatName(searchInput)));
    }
    return heroes;
}
...
function Heroes({ heroes, searchInput, onClickFn }) {
    ...
    return heroes?.length > 0 ?
        <div className="heroes-list">
            {heroes?.map(hero => <HeroPreview key={hero?.id} heroId={hero?.id} img={hero?.img} name={hero?.localized_name} onClickFn={onClickFn} />)}
        </div>
        :
        <EmptyResultMessage />
}
function SearchBar({ value, onChange, handleSubmit }) {
    return (
        <form className="search-bar" onSubmit={handleSubmit}>
            <input value={value} onChange={e => onChange(e.target.value)} placeholder="Look for a hero by name" autoFocus />
        </form>
    )
}
function HeroesView({ onClickFn }) {
    ...
    return (
        <div className="heroes-view">
            <SearchBar value={searchInput} onChange={setSearchInput} handleSubmit={handleSubmit} />
